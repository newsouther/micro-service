...
import com.souther.cloud.constant.SecurityConstants;
import com.souther.cloud.constant.SecurityRedisKeyEnum;
...
public class CustomRedisTokenStore implements TokenStore {
  ...
  private static final String ACCESS = SecurityRedisKeyEnum.ACCESS.getKey();
  private static final String AUTH_TO_ACCESS = SecurityRedisKeyEnum.AUTH_TO_ACCESS.getKey();
  private static final String REFRESH_AUTH = SecurityRedisKeyEnum.REFRESH_AUTH.getKey();
  private static final String ACCESS_TO_REFRESH = SecurityRedisKeyEnum.ACCESS_TO_REFRESH.getKey();
  private static final String REFRESH = SecurityRedisKeyEnum.REFRESH.getKey();
  private static final String REFRESH_TO_ACCESS = SecurityRedisKeyEnum.REFRESH_TO_ACCESS.getKey();
...
//    }
  ...
  private int getAccessTokenValiditySeconds(String clientId) {
    ...
    try {
      bytes = conn.get(serializeKey(
          SecurityRedisKeyEnum.CACHE_CLIENT_KEY.getKey() + ":" + clientId));
    } finally {
      conn.close();
    }
    if (bytes != null) {
  ...
  public OAuth2Authentication readAuthentication(String token) {
    ...
    RedisConnection conn = getConnection();
    try {
      bytes = conn.get(serializeKey(SecurityRedisKeyEnum.REDIS_TOKEN_AUTH.getKey() + token));
    } finally {
      conn.close();
    }
  ...
  private void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication,
    ...
    byte[] authKey = serializeKey(SecurityRedisKeyEnum.REDIS_TOKEN_AUTH.getKey() + token.getValue());
    ...
    byte[] approvalKey = serializeKey(
        SecurityRedisKeyEnum.REDIS_UNAME_TO_ACCESS.getKey() + getApprovalKey(authentication));
    byte[] clientId = serializeKey(
        SecurityRedisKeyEnum.REDIS_CLIENT_ID_TO_ACCESS.getKey() + authentication.getOAuth2Request()
            .getClientId());
  ...
  public void removeAccessToken(String tokenValue) {
    ...
    byte[] authKey = serializeKey(SecurityRedisKeyEnum.REDIS_TOKEN_AUTH.getKey() + tokenValue);
  ...
  public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId,
      String userName) {
    byte[] approvalKey = serializeKey(
        SecurityRedisKeyEnum.REDIS_UNAME_TO_ACCESS.getKey() + getApprovalKey(clientId, userName));
    List<byte[]> byteList;
    RedisConnection conn = getConnection();
    try {
  ...
  public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
    byte[] key = serializeKey(SecurityRedisKeyEnum.REDIS_CLIENT_ID_TO_ACCESS.getKey() + clientId);
    List<byte[]> byteList;
    RedisConnection conn = getConnection();
    try {
